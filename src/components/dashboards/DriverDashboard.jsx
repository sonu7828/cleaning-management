        ...
        <div className="max-w-4xl mx-auto space-y-6 pb-12 text-slate-100 px-4">
            ...
            <div className="flex items-center gap-3 border-b border-white/5 pb-4">
                ...
                <div>
                    ...
                    <p className="text-slate-400 text-xs mt-0.5">Toyota Hiace - Van #04 | Active Duty</p>
                </div>
            </div>
            ...
            <div>
                ...
                <p className="text-xs text-slate-400">Select an assigned cleaning roster route below to start transit.</p>
            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                                ...
                                <div className="space-y-1 md:col-span-4">
                                    <span className={`inline-block px-2.5 py-0.5 rounded-full text-[9px] font-black border ${
                                        route.status === 'Completed' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                                        route.status === 'In Progress' ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20 animate-pulse' :
                                        'bg-amber-500/10 text-amber-400 border-amber-500/20'
                                    }`}>
                                        {route.status === 'In Progress' ? 'In Transit' : route.status}
                                    </span>
                                    <h3 className="font-extrabold text-sm sm:text-base text-white leading-snug">{route.customerName}</h3>
                                    <p className="text-[10px] text-slate-400 font-semibold">Route ID: {route.id}</p>
                                </div>
                                ...
                                <div className="space-y-2 text-xs text-slate-600 md:col-span-4">
                                    ...
                                    {route.instructions && (
                                        <div className="mt-1.5 p-2 bg-slate-900/60 border border-white/5 rounded-xl text-[10px] text-slate-400 leading-relaxed">
                                            <strong className="text-slate-300">Special Instructions:</strong> {route.instructions}
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex md:justify-end md:col-span-4">
                                    ...
                                    {route.status === 'In Progress' && (
                                        <button
                                            ...
                                            className="w-full md:w-auto px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold rounded-xl flex items-center justify-center gap-1.5 text-xs active:scale-[0.98] transition-all shadow-lg shadow-emerald-600/10"
                                        >
                                    ...
                                    {route.status === 'Completed' && (
                                        <div className="w-full md:w-auto px-5 py-2.5 bg-white/5 border border-white/5 text-slate-400 font-extrabold rounded-xl flex items-center justify-center gap-1.5 text-xs">
                                            <FaCheckCircle className="text-emerald-400 text-[10px]" /> Drop-off Completed
                                        </div>
                                    )}
                                </div>
                            </div>
